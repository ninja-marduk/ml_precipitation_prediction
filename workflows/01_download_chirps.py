...
def main():
    parser = argparse.ArgumentParser(
        description='Download and crop CHIRPS data',
        epilog="""
Region coordinates can be specified via CLI flags or config.yaml.
CLI flags override config.yaml values when both are provided.

Examples:
  # Use default Boyaca region from config.yaml
  python workflows/01_download_chirps.py

  # Override region via CLI (e.g., for a different study area)
  python workflows/01_download_chirps.py \\
      --lon-min -76.0 --lon-max -73.0 \\
      --lat-min 2.0 --lat-max 5.0

  # Custom CHIRPS directory + BSC config
  python workflows/01_download_chirps.py \\
      --config .barcelona/config_bsc.yaml \\
      --chirps-dir /gpfs/projects/bsc32/chirps
        """,
    ...
    parser.add_argument('--lon-min', type=float, default=None,
                        help='Western longitude bound (default: -74.975 for Boyaca)')
    parser.add_argument('--lon-max', type=float, default=None,
                        help='Eastern longitude bound (default: -71.725 for Boyaca)')
    parser.add_argument('--lat-min', type=float, default=None,
                        help='Southern latitude bound (default: 4.325 for Boyaca)')
    parser.add_argument('--lat-max', type=float, default=None,
                        help='Northern latitude bound (default: 7.375 for Boyaca)')
    ...
    lon_min = args.lon_min if args.lon_min is not None else region['lon_min']
    lon_max = args.lon_max if args.lon_max is not None else region['lon_max']
    lat_min = args.lat_min if args.lat_min is not None else region['lat_min']
    lat_max = args.lat_max if args.lat_max is not None else region['lat_max']
