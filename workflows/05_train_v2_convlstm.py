...
class V2Config:
    ...
    def from_yaml(cls, config_path: Path, intracell_dem: bool = False,
        ...
        if intracell_dem:
            ...
            obj.output_dir = ic.get('v2_output_dir',
                                    'models/output/intracell_dem/ConvLSTM')
            if bundle:
    ...
    def default(cls, intracell_dem: bool = False, bundle: str = None) -> 'V2Config':
        ...
        if intracell_dem:
            ...
            obj.output_dir = 'models/output/intracell_dem/ConvLSTM'
            if bundle:
                obj.feature_sets = {bundle: ALL_BUNDLES[bundle]}
            else:
                obj.feature_sets = dict(FEATURE_SETS_INTRACELL)
...
def setup_tensorflow(seed=42, allow_missing=False):
    ...
    try:
        ...
        if gpus:
            ...
            # Restrict to single GPU to avoid multi-GPU CUDA handle issues
            tf.config.set_visible_devices(gpus[0], 'GPU')
            logger.info(f'GPU available: {len(gpus)} device(s), using GPU:0')
            # Warm-up: force CUDA context initialization before model building
...
def define_custom_objects(tf):
    ...
    class MultiHorizonLoss(Loss):
        ...
        def __init__(self, horizon_weights=None, name='multi_horizon_loss', **kwargs):
            ...
            self._hw_list = list(horizon_weights)
            self.horizon_weights = _tf.constant(horizon_weights, dtype=_tf.float32)

        def call(self, y_true, y_pred):
            import tensorflow as _tf
            y_pred = _tf.maximum(y_pred, 0.0)
            mse = _tf.reduce_mean(_tf.square(y_true - y_pred), axis=[2, 3, 4])
            weights = _tf.reshape(self.horizon_weights, [1, -1])
            return _tf.reduce_mean(_tf.reduce_sum(mse * weights, axis=1))

        def get_config(self):
            config = super().get_config()
            config.update({'horizon_weights': self._hw_list})
            return config
    ...
    class CombinedLoss(Loss):
        ...
        def call(self, y_true, y_pred):
            ...
            mse = _tf.reduce_mean(_tf.square(y_true - y_pred), axis=[2, 3, 4])
            weights = _tf.reshape(self.horizon_weights, [1, -1])
            mh_loss = _tf.reduce_mean(_tf.reduce_sum(mse * weights, axis=1))
            temporal_diffs = _tf.abs(y_pred[:, 1:, :, :, :] - y_pred[:, :-1, :, :, :])
            tc_loss = _tf.reduce_mean(temporal_diffs)
            return mh_loss + self.consistency_weight * tc_loss

        def get_config(self):
            config = super().get_config()
            config.update({
                'horizon_weights': self._hw_list,
    ...
    class SpatialReshapeLayer(Layer):
        ...
        def call(self, inputs):
            import tensorflow as _tf
            batch_size, time_steps, height, width, channels = _tf.unstack(_tf.shape(inputs))
            return _tf.reshape(inputs, [batch_size, time_steps, height * width * channels])
    ...
    class SpatialRestoreLayer(Layer):
        ...
        def call(self, inputs):
            import tensorflow as _tf
            batch_size, time_steps, _ = _tf.unstack(_tf.shape(inputs))
            return _tf.reshape(inputs, [batch_size, time_steps,
...
def build_conv_lstm(tf, n_feats, lat, lon, horizon):
    ...
    # Spatial head: Conv2D(horizon) -> permute (H,W,C)->(C,H,W) -> reshape
    x = Conv2D(horizon, (1, 1), padding='same', activation='linear',
               name='head_conv1x1')(x)
    # x shape: (batch, lat, lon, horizon) -> need (batch, horizon, lat, lon, 1)
    x = Permute((3, 1, 2), name='head_transpose')(x)
    x = Reshape((horizon, lat, lon, 1), name='head_expand_dim')(x)
...
def load_and_validate_data(config: V2Config, base_path: Path):
    ...
    # Try multiple engines (netCDF4, h5netcdf, scipy) for cross-environment compatibility
    ds = None
    last_error = None
    ...
    if ds is None:
        raise RuntimeError(
            f'Could not open {data_file} with any engine. Last error: {last_error}'
        )
