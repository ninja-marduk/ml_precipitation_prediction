...
# Ruta del dataset
DATASET_PATH = "/Users/riperez/Conda/anaconda3/doc/precipitation/output/boyaca_region_monthly.nc"
OUTPUT_PATH = "/Users/riperez/Conda/anaconda3/doc/precipitation/output/boyaca_region_monthly_aggregated.nc"
...
def aggregate_monthly_average(ds):
    """
    Aggregate the dataset by calculating the monthly average across all years.

    Parameters:
        ds (xarray.Dataset): The dataset to aggregate.

    Returns:
        xarray.Dataset: Aggregated dataset with monthly averages.
    """
    try:
        ...
        monthly_avg = monthly_avg.rename({"month": "month_index"})
        monthly_avg.attrs["description"] = "Monthly average aggregated across all years"
...
def generate_monthly_maps(ds, output_dir):
    """
    Generate a map for each month showing the precipitation values for each coordinate.

    Parameters:
        ds (xarray.Dataset): The aggregated dataset with monthly averages.
...
def generate_combined_monthly_map(ds, output_path):
    """
    ...
    try:
        ...
        plt.suptitle("Monthly Precipitation Averages", fontsize=16)
...
def main():
    ...
    try:
        ...
        monthly_avg_ds = aggregate_monthly_average(ds)

        logger.info("Saving aggregated dataset...")
        save_dataset(monthly_avg_ds, OUTPUT_PATH)

        logger.info("Generating combined map for all months...")
        combined_map_path = os.path.join(MAPS_OUTPUT_DIR, "combined_monthly_map.png")
        generate_combined_monthly_map(monthly_avg_ds, combined_map_path)
