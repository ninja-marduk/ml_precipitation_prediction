...
def load_predictions(config: V10Config, base_path: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    paths = {
        'V2 ConvLSTM': base_path / config.v2_predictions_path,
        'V4 GNN-TAT': base_path / config.v4_predictions_path,
        'Targets': base_path / config.targets_path,
    }
    missing = [name for name, p in paths.items() if not p.exists()]
    if missing:
        for name in missing:
            logger.error(f'  MISSING: {name} -> {paths[name]}')
        raise FileNotFoundError(
            f'Missing prediction files: {", ".join(missing)}. '
            f'Run stages 5-6 first.')

    v2_pred = np.load(paths['V2 ConvLSTM'])
    logger.info(f'  V2 ConvLSTM: {v2_pred.shape}')

    v4_pred = np.load(paths['V4 GNN-TAT'])
...
def main():
    ...
    # Intracell DEM mode: override paths to new structure with latest_run()
    if args.intracell_dem:
        bundle = args.bundle
        config.feature_set = bundle
        idem = Path(PROJECT_ROOT) / 'models' / 'output' / 'intracell_dem'
        ...
        v2_run = _latest_run(idem / 'ConvLSTM' / bundle)
        v4_run = _latest_run(idem / 'GNN_TAT_GAT' / bundle)
        if v2_run:
            config.v2_predictions_path = str(v2_run / 'predictions.npy')
            config.targets_path = str(v2_run / 'targets.npy')
        if v4_run:
            config.v4_predictions_path = str(v4_run / 'predictions.npy')
        config.output_dir = str(idem / 'Late_Fusion' / bundle)
