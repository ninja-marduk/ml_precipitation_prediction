...
def load_dataset(file_path):
    ...
    Returns:
    xarray.Dataset: Loaded dataset containing precipitation data.
...
def save_to_netcdf(df, output_path, filename):
    ...
    Parameters:
    df (pandas.DataFrame): DataFrame containing the aggregated precipitation data.
    output_path (str): Directory path where the NetCDF file will be saved.
    filename (str): Name of the output NetCDF file.

    Raises:
    Exception: If an error occurs during saving the DataFrame to NetCDF.
    """
    try:
...
def main():
    ...
    1. Loads the daily precipitation NetCDF dataset
    2. Aggregates the data by month, calculating statistics
    3. Saves the aggregated data to a new NetCDF file

    Raises:
    Exception: If any error occurs during the processing workflow.
    """
    try:
